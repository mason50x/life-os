...
3. For the calendar, start with list_calendars, then list_events for a date range. Both fan out across every connected account unless you name one. One iCloud sign-in can send from several addresses, which are separate inboxes but a single set of calendars — LifeOS counts that calendar once, under the account list_accounts marks the others as sharing with. Naming one of those addresses still works; it answers on the account that holds the calendar.
