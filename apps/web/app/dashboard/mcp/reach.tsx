...
import type { ConnectedAccount } from "@lifeos/core/accounts";
import { CapabilityBadges, ProviderMark } from "@/components/dashboard/ProviderMark";
import { useLiveAccounts } from "@/components/dashboard/live";
...
/**
 * The two sections that depend on what's connected, subscribed to Convex: the
 * endpoint gates its toolset on the account list at call time, so this page
 * follows the same list live — connect an account in another tab and the
 * tools it unlocks appear here without a refresh.
 */
export function McpReachLive({ initialAccounts }: { initialAccounts: ConnectedAccount[] }) {
  const accounts: ConnectedAccount[] = useLiveAccounts(initialAccounts);

  // What this connection really advertises, not the full catalogue.
          ...
          {accounts.length === 0 ? (
            ...
            accounts.map((a, i) => (
              ...
              >
                ...
                {a.status === "active" ? (
                  <CapabilityBadges capabilities={a.capabilities} calendarOf={a.calendarOf} />
                ) : (
